    ...
    async def _save_metrics(self, metrics: Dict[str, Any]) -> None:
        ...
        query = """
            INSERT INTO stock_screening_metrics (
                ...
                stoch_k, stoch_d, cci_20, adx_14, plus_di, minus_di,
                ao_value, momentum_10, stoch_rsi_k, stoch_rsi_d,
                williams_r, bull_power, bear_power, ultimate_osc,
                ema_10, ema_30, ema_50, ema_100, ema_200,
                sma_10, sma_30, sma_100, ichimoku_base, vwma_20,
                tv_osc_buy, tv_osc_sell, tv_osc_neutral,
                tv_ma_buy, tv_ma_sell, tv_ma_neutral,
                tv_overall_signal, tv_overall_score
            ) VALUES (
            ...
            DO UPDATE SET
                ...
                bull_power = EXCLUDED.bull_power,
                bear_power = EXCLUDED.bear_power,
                ultimate_osc = EXCLUDED.ultimate_osc,
                ...
                ichimoku_base = EXCLUDED.ichimoku_base,
                vwma_20 = EXCLUDED.vwma_20,
                tv_osc_buy = EXCLUDED.tv_osc_buy,
                tv_osc_sell = EXCLUDED.tv_osc_sell,
                tv_osc_neutral = EXCLUDED.tv_osc_neutral,
                tv_ma_buy = EXCLUDED.tv_ma_buy,
                tv_ma_sell = EXCLUDED.tv_ma_sell,
                tv_ma_neutral = EXCLUDED.tv_ma_neutral,
                tv_overall_signal = EXCLUDED.tv_overall_signal,
                tv_overall_score = EXCLUDED.tv_overall_score,
                created_at = NOW()
        """
        ...
        await self.db.execute(
            ...
            metrics.get('ao_value'),
            metrics.get('momentum_10'),
            metrics.get('stoch_rsi_k'),
            metrics.get('stoch_rsi_d'),
            metrics.get('williams_r'),
            metrics.get('bull_power'),
            metrics.get('bear_power'),
            metrics.get('ultimate_osc'),
            ...
            metrics.get('sma_30'),
            metrics.get('sma_100'),
            metrics.get('ichimoku_base'),
            metrics.get('vwma_20'),
            # TradingView Summary
            metrics.get('tv_osc_buy'),
            metrics.get('tv_osc_sell'),
            metrics.get('tv_osc_neutral'),
            metrics.get('tv_ma_buy'),
            metrics.get('tv_ma_sell'),
            metrics.get('tv_ma_neutral'),
            metrics.get('tv_overall_signal'),
            metrics.get('tv_overall_score'),
        )
