...
log_handler = TimedRotatingFileHandler("/app/logs/smc1_alert.log", when="midnight", interval=1, backupCount=7)
    ...
    for epic in epics:
        ...
        if not alerts:
            ...
            if now - alert_time > timedelta(minutes=30):
                ...
                print(f"⏩ Alert for {epic} at {alert_time} ({alert['type']}) already exists in DB, skipping.")
            else:
            ...
            print(f"📏 Using ATR-based stop: {ig_stop_points} IG points")

            print(f"📤 Sending order: {epic} {direction} @ {alert['price']} with stop {ig_stop_points}")
