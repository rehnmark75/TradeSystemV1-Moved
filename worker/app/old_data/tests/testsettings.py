...
log_handler = TimedRotatingFileHandler("/app/logs/modular_alert.log", when="midnight", interval=1, backupCount=7)
...
def detect_ema12_50_cross_confirmed(df: pd.DataFrame, eps: float = 1e-8) -> pd.DataFrame:
    df = df.sort_values("start_time").reset_index(drop=True)
    ...
    df["bull_ema_cross"] = (
        (df["prev_ema_12"] < df["prev_ema_50"] - eps) &
        (df["ema_12"] > df["ema_50"] + eps) &
        (df["next_close"] > df["ema_12"] + eps)
    )
...
def is_ema_distance_exceeded(row, ema_fast_col="ema_12", ema_slow_col="ema_50", max_pct: float = 0.5) -> bool:
    """
    ...
    - max_pct (float): Max allowed EMA distance as a percentage of the slow EMA.

    Returns:
    - bool: True if distance is too large.
    """
    fast = row.get(ema_fast_col, None)
    slow = row.get(ema_slow_col, None)
...
def calculate_atr(df, length=14):
    ...
    return df["tr"].rolling(window=length).mean().iloc[-1]
...
def run_alert_pipeline(epic: str, engine, debug=False) -> tuple[list[dict], pd.DataFrame]:
    ...
    df_15m["ema_12"] = compute_ema(df_15m["close"], 12)
    df_15m["ema_50"] = compute_ema(df_15m["close"], 50)
    df_15m = compute_alligator(df_15m)
    df_15m = compute_macd(df_15m, suffix="_15m")

    # detect EMA alerts
    df_15m = detect_ema_alerts_from_indicators(df_15m)
    df_15m = detect_ema12_50_cross_confirmed(df_15m)

    df_15m = slope_diff_1(df_15m, "ema_12", out_col="ema12_slope1")
    df_15m = slope_diff_2(df_15m, "ema_12", out_col="ema12_slope2")
    df_15m = slope_linregress(df_15m, "ema_12", window=3, out_col="ema12_slope_lr") 
...
def main():    
    ...
    for epic in epics:
        ...
        df_result, df_1h = run_alert_pipeline(epic, engine, debug=True)
        alerts = detect_valid_alerts(df_result , debug=False,use_macd_5m_filter=False, use_macd_15m_filter=False, use_macd_1h_filter=False,
                                    ...
                                    slope_method="linreg",use_ema_distance_filter=True, max_ema_distance_pct=0.05,
                                    use_follow_through_filter=True)
        ...
        if alerts:
            ...
            last_alert = alerts[-1]
            log.info(f"\n📌 Last alert: {last_alert['type'].upper()} at {last_alert['price']} ({last_alert['timestamp']})")
            
            #img_path = plot_alert_chart(chart_df, epic, 15, image_path,last_alert=last_alert)
        else:
        ...
        if last_alert:
            ...
            if now - alert_time > timedelta(minutes=30):
                 print(f"⏩ Alert for {epic} is older than 30 minutes ({alert_time}), skipping.")
                 continue
        ...
        # Check if already in DB
            ...
            try:
                ...
                log.info(f"Failed to log alert to database: {e}")
            
    
            atr_1h = df_1h["atr_1h"].iloc[-1]
