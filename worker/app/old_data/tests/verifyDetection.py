...
def detect_ema_alerts(df):
    ...
    # EMA slope
    df['ema_12_slope'] = df['ema_12'] - df['ema_12'].shift(1)
    ...
    # MACD-based filters
    df['macd_bull'] = df['macd_histogram'] > 0
    df['macd_bear'] = df['macd_histogram'] < 0

    # Alerts with MACD confirmation
    df['bull_alert'] = df['bull_cross'] & df['bull_condition'] & df['macd_bull']
    df['bear_alert'] = df['bear_cross'] & df['bear_condition'] & df['macd_bear']
...
def main():
    ...
    for epic in epics:
        ...
        try:
            ...
            df = fetch_candle_data(engine, epic, timeframe, 500)
            ...
            signals = get_structure_signals(df)  # or from your alert logic
            confirmed_swings = mark_confirmed_swings(classified, signals)

            # Step 2: Extract signals (from full price dataframe)
            signals = get_structure_signals(classified)  # This must pull CHoCH/BOS alerts based on logic (not from classified)
            
            # DEBUG: Print the extracted structure signals
            # Step 3: Mark which pivots were confirmed by signals
            confirmed_swings = mark_confirmed_swings(classified, signals)

            # Step 4: Display the confirmed pivots
            confirmed = [p for p in confirmed_swings if p.get("confirmed")]
            
            # Step 3: Get CHoCH/BOS signal list from your alert detection logic
            signals = get_structure_signals(df)  # assuming you already have this function

            # Step 4: Mark confirmed swings
            confirmed_swings = mark_confirmed_swings(classified, signals)

            # Step 5: Extract most recent confirmed HH/LL from those
            extremes = extremes = get_most_recent_confirmed_hh_ll(confirmed_swings) or get_recent_trailing_extremes(classified)

            latest_confirmed_time = max(p['time'] for p in confirmed if p['label'] in ['HH', 'LL'])

            # Find unconfirmed but newer HH/LL swings
            fallback_hh = next((p for p in reversed(classified) if p['label'] == 'HH' and p['time'] > latest_confirmed_time), None)
            fallback_ll = next((p for p in reversed(classified) if p['label'] == 'LL' and p['time'] > latest_confirmed_time), None)

            # Prefer confirmed extremes, but fallback to unconfirmed if newer
            top_source = extremes.get("top_source") or fallback_hh
            bottom_source = extremes.get("bottom_source") or fallback_ll

            if top_source and bottom_source:
                top = top_source["price"]
                bottom = bottom_source["price"]
            else:
                log.warning("⚠️ Still no HH/LL structure found")
                continue
            if extremes:
                top_source = extremes.get("top_source")
                bottom_source = extremes.get("bottom_source")
                if top_source:
                    print(f"🔺 HH used for Premium: {top_source['price']} at {top_source['time']}")
                else:
                    print("❌ No HH found")
                if bottom_source:
                    print(f"🔻 LL used for Discount: {bottom_source['price']} at {bottom_source['time']}")
                else:
                    print("❌ No LL found")
            else:
                log.warning(f"⚠️ No confirmed HH/LL structure found for {epic}")
            ...
            # Step 6: Use `extremes["top"]` and `extremes["bottom"]` for zone calculations
            top = extremes["top"]
            bottom = extremes["bottom"]
            midpoint = (top + bottom) / 2

            for pivot in reversed(classified):
                if top is None and pivot["label"] == "HH":
                    top = pivot["price"]
                    top_source = pivot
                elif bottom is None and pivot["label"] == "LL":
                    bottom = pivot["price"]
                    bottom_source = pivot
                if top and bottom:
                    break
            ...
            if not top or not bottom:
                log.warning(f"⚠️ No valid trailing HH/LL found for {epic} — top={top}, bottom={bottom}")
                continue

            # --- Midpoint-based validation logic ---
            midpoint = (top + bottom) / 2
            ...
            for _, row in df.iterrows():
                if not row['bull_alert'] and not row['bear_alert']:
                    continue

                alert_type = "bull" if row['bull_alert'] else "bear"
                alert_price = float(row['close'])
                alert_time = row['start_time']

                # Midpoint-based validation
                if alert_type == "bull":
                    if alert_price < midpoint:
                        position_note = "Below Midpoint (Discount Zone) ✅"
                        zone_confirmed = True
                    else:
                        position_note = "Above Midpoint (Invalid for Bull) ❌"
                        zone_confirmed = False
                else:
                    if alert_price > midpoint:
                        position_note = "Above Midpoint (Premium Zone) ✅"
                        zone_confirmed = True
                    else:
                        position_note = "Below Midpoint (Invalid for Bear) ❌"
                        zone_confirmed = False

                print(f"📊 Top: {top}, Bottom: {bottom}")
                print(f"📊 Midpoint: {midpoint}")
                print(f"📍 Alert Price: {alert_price} — Position: {position_note}")
                print(f"[{alert_time}] {'🟢' if alert_type == 'bull' else '🔴'} {alert_type.title()} Alert | Zone Confirmed: {zone_confirmed}")
            ...
            if confirmed_alerts:
                ...
                for swing in confirmed_swings:
                    ...
                    match_idx = chart_df[chart_df["start_time"] == swing_time].index
                    if not match_idx.empty:
                        chart_df.at[match_idx[0], "structure"] = swing["label"]
                img_path = plot_alert_chart(chart_df, epic, timeframe, image_path, position_note=position_note)
