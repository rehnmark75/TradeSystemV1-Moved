    ...
    for idx, row in df.iterrows():
        ...
        if bull_trigger:
            ...
            if use_zone_filter and zones and alert_price > zones.get("equilibrium_top", float("inf")):
                reasons.append("Price not in discount zone")
                score -= 2

            if use_ema_distance_filter and pd.notnull(row.get(fast_ema_col)) and pd.notnull(row.get(slow_ema_col)):
                if is_ema_distance_exceeded(row, fast_ema_col, slow_ema_col, max_pct=max_ema_distance_pct):
                    reasons.append("EMA distance too wide")
                    score -= 1

            if use_local_extreme_filter and is_near_local_extreme(df, idx, direction="bull", atr_mult=atr_mult_extreme):
                reasons.append("Too close to recent local high")
                score -= 2
            ...
            if use_trend_filter:
                if row.get("trend", False):
                    score += 1
                else:
                    reasons.append("Trend not bullish")
                    score -= 2

            if use_macd_5m_filter:
                score += 1 if row.get("macd_histogram_5m", 0) > 0 else -1

            if use_macd_15m_filter:
                score += 1 if row.get("macd_histogram_15m", 0) > 0 else -1

            if use_macd_1h_filter:
                score += 1 if row.get("macd_histogram_1h", 0) > 0 else -1

            if use_exhaustion_filter and is_exhaustion_candle(row["open"], row["high"], row["low"], row["close"]):
                reasons.append("Exhaustion candle")
                score -= 1

            if use_overextension_filter and pd.notnull(row.get(fast_ema_col)):
                if is_overextended(row["close"], row[fast_ema_col], threshold_pct=1.5):
                    reasons.append("Overextended from fast EMA")
                    score -= 1

            if use_slope_filter:
                score += 1 if row.get(slope_col_bull, False) else -1

            if use_follow_through_filter:
                score += 1 if confirms_strong_direction(df, idx, "bull") else -1
        ...
        if bear_trigger:
            ...
            if use_zone_filter and zones and alert_price < zones.get("equilibrium_bottom", -float("inf")):
                reasons.append("Price not in premium zone")
                score -= 2
            ...
            if use_local_extreme_filter and is_near_local_extreme(df, idx, direction="bear", atr_mult=atr_mult_extreme):
                reasons.append("Too close to recent local low")
                score -= 2
            ...
            if use_slope_filter:
                score += 1 if row.get(slope_col_bear, False) else -1

            if use_follow_through_filter:
                score += 1 if confirms_strong_direction(df, idx, "bear") else -1

        if alert_type and score >= min_alert_score:
            alerts.append({
...
def select_lowest_LL(swings_df: pd.DataFrame, bar_limit: int = 192, debug: bool = False):
    recent_swings = swings_df.tail(bar_limit).copy()
    ll_swings = recent_swings[recent_swings["label"] == "LL"]
    ...
    if debug:
        print("⚠️ No LL found in the recent bars.")
    return None
...
if __name__ == "__main__":
   ...
   # Step 1: Detect swing pivots using the LuxAlgo-style function
    ...
    alerts = detect_scored_alerts(df_result , debug=True,use_macd_5m_filter=False, use_macd_15m_filter=False, use_macd_1h_filter=False,
