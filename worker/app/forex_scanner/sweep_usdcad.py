#!/usr/bin/env python3
"""
USDCAD Parameter Sweep v2.47.0
Tests: ATR ref, MFI, swing significance, min ADX, max confidence,
       regime SL/TP, ATR normalization, blocked hours, and combinations.

Current USDCAD: SL=12 TP=18, max_conf=0.64, blocked hours=14,15
Already has: scalp_min_adx=20, RSI filter, block ranging+low_vol_trending
1h ATR ~9 pips (similar to EURUSD default ref=8).
"""
import subprocess
import json
import re
...
DAYS = 30
TIMEFRAME = '5m'
PROGRESS_FILE = '/tmp/sweep_usdcad_progress.log'
RESULTS_FILE = '/tmp/sweep_usdcad_results.txt'
...
def main():
    ...
    log(f"=== USDCAD Parameter Sweep ({DAYS}d, {TIMEFRAME}) — {TOTAL_TESTS} tests ===\n")
    ...
    # USDCAD 1h ATR ~9 pips, close to EURUSD default ref=8
    log("\n--- GROUP 2: ATR Reference Pips ---")
    for atr_ref in [8, 9, 10, 12]:
        restore_state(original)
    ...
    for adx in [0, 18, 20, 22, 25, 28]:
        ...
        if adx > 0:
            ...
            # Remove the existing scalp_min_adx to test with no filter
            with get_conn() as conn, conn.cursor() as cur:
                cur.execute("UPDATE smc_simple_pair_overrides SET parameter_overrides = parameter_overrides - 'scalp_min_adx' WHERE epic=%s", (EPIC,))
                conn.commit()
    ...
    restore_state(original)
    apply_db_override(extra_params={'regime_sl_tp_enabled': False, 'atr_normalized_sl_tp_enabled': True, 'atr_reference_pips': 9.0})
    n += 1; r = run_backtest("7b. ATR norm only (ref=9)", n, TOTAL_TESTS)
    results.append(r); log(fmt(r))

    restore_state(original)
    apply_db_override(extra_params={'regime_sl_tp_enabled': True, 'atr_normalized_sl_tp_enabled': True, 'atr_reference_pips': 9.0})
    n += 1; r = run_backtest("7c. Both: regime+ATR(9)", n, TOTAL_TESTS)
    results.append(r); log(fmt(r))
    ...
    restore_state(original)
    apply_db_override(extra_params={'regime_sl_tp_enabled': True, 'atr_normalized_sl_tp_enabled': True, 'atr_reference_pips': 9.0, 'atr_scale_min': 0.8, 'atr_scale_max': 1.3})
    n += 1; r = run_backtest("7e. Both(9) scale 0.8-1.3", n, TOTAL_TESTS)
    results.append(r); log(fmt(r))

    restore_state(original)
    apply_db_override(extra_params={'regime_sl_tp_enabled': True, 'atr_normalized_sl_tp_enabled': True, 'atr_reference_pips': 9.0,
                                     'trending_sl_mult': 1.0, 'trending_tp_mult': 1.2, 'ranging_sl_mult': 1.1, 'ranging_tp_mult': 0.8})
    n += 1; r = run_backtest("7f. Both(9)+regime mults", n, TOTAL_TESTS)
    results.append(r); log(fmt(r))

    # ─── GROUP 8: BLOCKED HOURS ──────────────────────────────────────────────
    log("\n--- GROUP 8: Blocked Hours Sweep ---")
    hour_configs = [
        ('14,15', 'current (block overlap)'),
        ('0,1,2,3,14,15,22,23', 'cur+deep Asian'),
        ('11,14,15', 'cur+hour11'),
        ('7,8,14,15', 'cur+early London'),
        ('14,15,16,17', 'cur+late NY'),
        ('', 'no blocks'),
    ]
    ...
    apply_db_override(extra_params={'mfi_filter_mode': 'ACTIVE', 'mfi_min_slope': -15.0,
                                     'swing_significance_filter_mode': 'ACTIVE', 'min_swing_significance': 0.40,
                                     'atr_normalized_sl_tp_enabled': True, 'atr_reference_pips': 9.0})
    n += 1; r = run_backtest("9b. MFI-15+SwSig0.40+ATR9", n, TOTAL_TESTS)
    results.append(r); log(fmt(r))

    restore_state(original)
    apply_db_override(extra_params={'mfi_filter_mode': 'ACTIVE', 'mfi_min_slope': -15.0,
                                     'swing_significance_filter_mode': 'ACTIVE', 'min_swing_significance': 0.40,
                                     'regime_sl_tp_enabled': True, 'atr_normalized_sl_tp_enabled': True, 'atr_reference_pips': 9.0})
    n += 1; r = run_backtest("9c. All+regime+ATR(9)", n, TOTAL_TESTS)
    results.append(r); log(fmt(r))
    ...
    log(f"📁 Full log at {PROGRESS_FILE}")
