#!/usr/bin/env python3
"""
AUDUSD Parameter Sweep v2.47.0
Tests: ATR ref, MFI, swing significance, min ADX, max confidence,
       regime SL/TP, ATR normalization, blocked hours, and combinations.

Output: writes progress to /tmp/sweep_audusd_progress.log inside container
        and final results to /tmp/sweep_audusd_results.txt
"""
import subprocess
import json
import re
...
DAYS = 30
TIMEFRAME = '5m'
PROGRESS_FILE = '/tmp/sweep_audusd_progress.log'
RESULTS_FILE = '/tmp/sweep_audusd_results.txt'
...
def log(msg):
    """Print and append to progress file."""
...
def run_backtest(label: str, test_num: int, total: int) -> dict:
    cmd = ['python', '/app/forex_scanner/bt.py', 'AUDUSD', str(DAYS), '--scalp', '--timeframe', TIMEFRAME]
    ...
    # Reconstruct WR from winners/signals if WR parsing failed
...
def main():
    ...
    log(f"=== AUDUSD Parameter Sweep ({DAYS}d, {TIMEFRAME}) — {TOTAL_TESTS} tests ===\n")
    ...
    restore_state(original)
    apply_db_override(extra_params={'regime_sl_tp_enabled': False, 'atr_normalized_sl_tp_enabled': True, 'atr_reference_pips': 10.0})
    n += 1; r = run_backtest("7b. ATR norm only (ref=10)", n, TOTAL_TESTS)
    results.append(r); log(fmt(r))

    restore_state(original)
    apply_db_override(extra_params={'regime_sl_tp_enabled': True, 'atr_normalized_sl_tp_enabled': True, 'atr_reference_pips': 10.0})
    n += 1; r = run_backtest("7c. Both: regime+ATR(10)", n, TOTAL_TESTS)
    results.append(r); log(fmt(r))
    ...
    restore_state(original)
    apply_db_override(extra_params={'regime_sl_tp_enabled': True, 'atr_normalized_sl_tp_enabled': True, 'atr_reference_pips': 10.0, 'atr_scale_min': 0.8, 'atr_scale_max': 1.3})
    n += 1; r = run_backtest("7e. Both(10) scale 0.8-1.3", n, TOTAL_TESTS)
    results.append(r); log(fmt(r))

    restore_state(original)
    apply_db_override(extra_params={'regime_sl_tp_enabled': True, 'atr_normalized_sl_tp_enabled': True, 'atr_reference_pips': 10.0,
                                     'trending_sl_mult': 1.0, 'trending_tp_mult': 1.2, 'ranging_sl_mult': 1.1, 'ranging_tp_mult': 0.8})
    n += 1; r = run_backtest("7f. Both(10)+regime mults", n, TOTAL_TESTS)
    results.append(r); log(fmt(r))
    ...
    hour_configs = [
        ('0,5,6,22,23', 'current'),
        ('0,1,2,3,4,5,6,22,23', 'block deep Asian'),
        ('0,5,6,14,15,22,23', 'cur+14-15'),
        ('0,5,6,7,8,22,23', 'cur+early London'),
        ('0,5,6,11,22,23', 'cur+hour11'),
        ('', 'no blocks'),
    ]
    ...
    apply_db_override(extra_params={'mfi_filter_mode': 'ACTIVE', 'mfi_min_slope': -15.0,
                                     'swing_significance_filter_mode': 'ACTIVE', 'min_swing_significance': 0.40,
                                     'atr_normalized_sl_tp_enabled': True, 'atr_reference_pips': 10.0})
    n += 1; r = run_backtest("9b. MFI-15+SwSig0.40+ATR10", n, TOTAL_TESTS)
    results.append(r); log(fmt(r))
    ...
    apply_db_override(extra_params={'mfi_filter_mode': 'ACTIVE', 'mfi_min_slope': -15.0,
                                     'swing_significance_filter_mode': 'ACTIVE', 'min_swing_significance': 0.40,
                                     'scalp_min_adx': 20.0,
                                     'regime_sl_tp_enabled': True, 'atr_normalized_sl_tp_enabled': True, 'atr_reference_pips': 10.0})
    n += 1; r = run_backtest("9c. All+regime+ATR(10)", n, TOTAL_TESTS)
    results.append(r); log(fmt(r))
