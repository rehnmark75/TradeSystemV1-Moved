...
# File paths
BASELINE_FILE = '/app/forex_scanner/logs/backtest_signals/execution_1775/signal_decisions.csv'
TEST_A_FILE = '/app/forex_scanner/logs/backtest_signals/execution_1776/signal_decisions.csv'
...
def analyze_pair_distribution(baseline, test_a):
    ...
    # Statistical test for distribution equality (use chi2_contingency for proper test)
    ...
    contingency_table = [baseline_counts, test_a_counts]
...
def analyze_htf_trend_distribution(baseline, test_a):
    ...
    # Chi-square test (only use categories present in both)
    contingency_table = pd.DataFrame({
        'Baseline': [baseline_trends.get('BULL', 0), baseline_trends.get('BEAR', 0)],
        'TEST_A': [test_a_trends.get('BULL', 0), test_a_trends.get('BEAR', 0)]
    }, index=['BULL', 'BEAR'])

    try:
        chi2, p_value, dof, expected = chi2_contingency(contingency_table.values)
    except ValueError:
        # Fallback if chi2 test fails
...
def analyze_premium_discount_distribution(baseline, test_a):
    ...
    # Chi-square test (only use categories present in both)
    contingency_table = pd.DataFrame({
        'Baseline': [baseline_zones.get('premium', 0), baseline_zones.get('discount', 0)],
        'TEST_A': [test_a_zones.get('premium', 0), test_a_zones.get('discount', 0)]
