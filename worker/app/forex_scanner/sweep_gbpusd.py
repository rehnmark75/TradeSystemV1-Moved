#!/usr/bin/env python3
"""
GBPUSD Parameter Sweep v2.47.0
Current: SL=12 TP=18, no max_conf set, blocked=14,15, monitor_only=true
Has: two-pole filter, MACD filter, no rejection candle
1h ATR ~12 pips.
"""
import subprocess
import json
...
DAYS = 30
TIMEFRAME = '5m'
PROGRESS_FILE = '/tmp/sweep_gbpusd_progress.log'
RESULTS_FILE = '/tmp/sweep_gbpusd_results.txt'
...
def main():
    ...
    log(f"=== GBPUSD Parameter Sweep ({DAYS}d, {TIMEFRAME}) — {TOTAL_TESTS} tests ===\n")
    ...
    # GBPUSD 1h ATR ~12 pips
    log("\n--- GROUP 2: ATR Reference Pips ---")
    for atr_ref in [8, 10, 12, 15]:
        restore_state(original)
    ...
    restore_state(original)
    apply_db_override(extra_params={'regime_sl_tp_enabled': False, 'atr_normalized_sl_tp_enabled': True, 'atr_reference_pips': 12.0})
    n += 1; r = run_backtest("7b. ATR norm only (ref=12)", n, TOTAL_TESTS)
    results.append(r); log(fmt(r))

    restore_state(original)
    apply_db_override(extra_params={'regime_sl_tp_enabled': True, 'atr_normalized_sl_tp_enabled': True, 'atr_reference_pips': 12.0})
    n += 1; r = run_backtest("7c. Both: regime+ATR(12)", n, TOTAL_TESTS)
    results.append(r); log(fmt(r))

    restore_state(original)
    apply_db_override(extra_params={'regime_sl_tp_enabled': True, 'atr_normalized_sl_tp_enabled': True, 'atr_reference_pips': 15.0})
    n += 1; r = run_backtest("7d. Both: regime+ATR(15)", n, TOTAL_TESTS)
    results.append(r); log(fmt(r))

    restore_state(original)
    apply_db_override(extra_params={'regime_sl_tp_enabled': True, 'atr_normalized_sl_tp_enabled': True, 'atr_reference_pips': 12.0, 'atr_scale_min': 0.8, 'atr_scale_max': 1.3})
    n += 1; r = run_backtest("7e. Both(12) scale 0.8-1.3", n, TOTAL_TESTS)
    results.append(r); log(fmt(r))

    restore_state(original)
    apply_db_override(extra_params={'regime_sl_tp_enabled': True, 'atr_normalized_sl_tp_enabled': True, 'atr_reference_pips': 12.0,
                                     'trending_sl_mult': 1.0, 'trending_tp_mult': 1.2, 'ranging_sl_mult': 1.1, 'ranging_tp_mult': 0.8})
    n += 1; r = run_backtest("7f. Both(12)+regime mults", n, TOTAL_TESTS)
    results.append(r); log(fmt(r))

    # ─── GROUP 8: BLOCKED HOURS ──────────────────────────────────────────────
    log("\n--- GROUP 8: Blocked Hours Sweep ---")
    hour_configs = [
        ('14,15', 'current (overlap)'),
    ...
    apply_db_override(extra_params={'mfi_filter_mode': 'ACTIVE', 'mfi_min_slope': -15.0,
                                     'swing_significance_filter_mode': 'ACTIVE', 'min_swing_significance': 0.40,
                                     'regime_sl_tp_enabled': True, 'atr_normalized_sl_tp_enabled': False})
    n += 1; r = run_backtest("9b. MFI-15+SwSig0.40+Regime", n, TOTAL_TESTS)
    results.append(r); log(fmt(r))

    restore_state(original)
    apply_db_override(extra_params={'mfi_filter_mode': 'ACTIVE', 'mfi_min_slope': -15.0,
                                     'swing_significance_filter_mode': 'ACTIVE', 'min_swing_significance': 0.40,
                                     'regime_sl_tp_enabled': True, 'atr_normalized_sl_tp_enabled': True, 'atr_reference_pips': 12.0})
    n += 1; r = run_backtest("9c. All+regime+ATR(12)", n, TOTAL_TESTS)
    results.append(r); log(fmt(r))
