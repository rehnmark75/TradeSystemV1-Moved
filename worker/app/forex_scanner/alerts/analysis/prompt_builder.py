...
"""
    ...
    def _build_smc_momentum_prompt(self, signal: Dict, has_chart: bool = True) -> str:
        """
        Vision prompt for SMC_MOMENTUM (Liquidity Sweep + Rejection Wick).

        This strategy is structurally different from SMC_SIMPLE and MEAN_REVERSION:
        - Entry fires ON the sweep candle (or the bar immediately after)
        - The mechanic: price runs a prior H/L to hunt stops, then closes back inside
        - Validation: wick beyond pool level ≥ 50% of candle range, close back inside
        - HTF filter: 4H EMA50 confirms direction — BULL bias = take downside sweeps of lows
          (wick pokes below a prior low, closes back up); BEAR bias = upside sweeps of highs

        Claude must evaluate whether the sweep looks genuine (stop-hunt, not breakout).
        """
        try:
            epic = signal.get('epic', 'Unknown')
            pair = self._extract_pair(epic)
            ...
            # Pool type → human-readable label
            pool_labels = {
            ...
            # Describe sweep mechanic depending on direction
            if direction in ('BULL', 'BUY'):
                sweep_what = f"sweep of a prior LOW ({pool_label} at {self._format_price(pool_level)})"
                ...
                sweep_what = f"sweep of a prior HIGH ({pool_label} at {self._format_price(pool_level)})"
...
- A candle whose WICK extends clearly beyond the pool level ({self._format_price(pool_level)})
- The BODY closes back on the correct side of that level
- The wick is prominent — at least half the total candle range
- The swept level ({pool_label}) should be a visible prior high or low — multiple candles touched or respected it before
...
- ATR TP target looks achievable given surrounding structure

**Negative signs (REJECT):**
- {reject_if}
- The "wick" is barely visible — excess is within normal candle noise, not a genuine stop-hunt
- Price has been consolidating for many candles below/above the level (no fresh stop cluster)
- Multiple consecutive candle closes already through the level before this signal (trend break, not sweep)
- The entry is firing into obvious momentum (wide-range expansion candles driving away from entry)
"""
...
- In a 4H BULLISH trend: take LONG entries on downside sweeps of prior lows (stops hunted, then price reverses up)
- In a 4H BEARISH trend: take SHORT entries on upside sweeps of prior highs (stops hunted, then price reverses down)
...
- ❌ Do NOT reject because there is no volume spike — tick volume on FX 15m is unreliable (filter is OFF by default)
...
• Pool Level: {self._format_price(pool_level)}  (the prior H/L that was swept)
...
Your response MUST begin with the literal token `SCORE:` as the very first characters. No preamble.

SCORE: [1-10]
DECISION: [APPROVE/REJECT]
REASON: [2-3 sentences. Focus on: (a) 4H EMA50 alignment with the signal direction, (b) whether the sweep candle shows a genuine wick rejection (close back inside the level) vs. a real breakout, (c) whether the swept pool level looks meaningful (visible prior H/L with stop clusters). Do NOT comment on MACD, RSI, or volume.]

**SCORING GUIDELINES:**
- 8-10: 4H trend clearly aligned, prominent wick rejection (>60% of candle range), meaningful pool level with visible prior touches, clean close back inside
- 6-7: 4H trend aligned but choppy, or wick is present but modest (50-60%), pool level visible but less tested
- 4-5: 4H trend unclear/ranging, wick marginal, or pool level looks like noise
- 1-3: 4H trend opposes signal, or price closed through the level (not a sweep — a breakout), or no visible wick beyond pool

**AUTOMATIC REJECTION CRITERIA:**
- 4H EMA50 clearly slopes against the signal direction (bearish EMA for a BUY, bullish EMA for a SELL)
- Price closed on the wrong side of the swept pool level (real breakout, stop-hunt thesis invalid)
- Multiple consecutive candle closes beyond the pool level before this signal (sustained break, not a sweep)
- No visible wick — the sweep candle has a normal body with no meaningful extension past the pool

**NOT rejection criteria (do NOT apply these):**
- MACD against entry direction
- RSI overbought/oversold for the entry side
- Absence of volume spike
- Price being "extended" from the 4H EMA (sweeps happen at distal price by definition)

Be concise. Your assessment determines if real capital is tested on this mechanic."""

        except Exception as e:
            self.logger.error(f"Error building SMC_MOMENTUM prompt: {e}")
            return self._build_fallback_prompt(signal, {})

    def _build_impulse_fade_prompt(self, signal: Dict, has_chart: bool = True) -> str:
        """
...
• Risk:Reward: {float(reward_pips)/float(risk_pips):.2f}:1  (inverted by design — ~80% WR compensates)
...
SCORE: [1-10]
DECISION: [APPROVE/REJECT]
REASON: [2-3 sentences. Focus on: (a) whether the impulse candle stands out as an isolated large-body spike vs. part of sustained momentum, (b) any exhaustion signals on the impulse candle or immediately after (wick, hesitation), (c) whether price context (nearby S/R, prior ranging, session timing) supports a snap-back. Do NOT comment on HTF trend, RSI, MACD, or R:R ratio.]

**SCORING GUIDELINES:**
- 8-10: Isolated oversized spike with visible exhaustion (wick on impulse candle, next candle hesitates), no established momentum trend beforehand, clean late-US session context
- 6-7: Large impulse but follow-through is uncertain, or minor continuation before signal but impulse candle still stands out
- 4-5: Impulse is part of a developing trend, or body multiplier is marginal (just above threshold)
- 1-3: Strong continuation after the impulse, or multiple large candles in a row, or genuine breakout through key level

**AUTOMATIC REJECTION CRITERIA:**
- Clear follow-through: the candle after the impulse is also a large-body candle in the same direction
- Sustained momentum: 3+ large candles in the impulse direction without meaningful pullback
- Clean structural breakout: price was pressing against a key level and this candle broke it with body close through
...
Be concise. Your assessment determines if real capital is tested on this edge."""
            ...
            self.logger.error(f"Error building IMPULSE_FADE prompt: {e}")
            return self._build_fallback_prompt(signal, {})
