"""
EMA Double Confirmation Rejection History Manager

Manages storage and retrieval of EMA Double Confirmation strategy rejections for analysis.
...
class EMADoubleRejectionHistoryManager:
    """
    Manages storage and retrieval of EMA Double Confirmation strategy rejections.
    ...
    STAGE_NO_CROSSOVER = 'NO_CROSSOVER'
    STAGE_INSUFFICIENT_CONFIRMATIONS = 'INSUFFICIENT_CONFIRMATIONS'
    STAGE_SESSION_FILTER = 'SESSION_FILTER'
    STAGE_HTF_TREND = 'HTF_TREND'
    STAGE_FVG_FILTER = 'FVG_FILTER'
    STAGE_ADX_FILTER = 'ADX_FILTER'
    STAGE_RISK_VALIDATION = 'RISK_VALIDATION'
    STAGE_CONFIDENCE = 'CONFIDENCE'

    STAGE_IMPULSE_QUALITY = 'IMPULSE_QUALITY'

    VALID_STAGES = [
        STAGE_DATA_VALIDATION, STAGE_NO_CROSSOVER, STAGE_INSUFFICIENT_CONFIRMATIONS,
        STAGE_SESSION_FILTER, STAGE_HTF_TREND, STAGE_FVG_FILTER, STAGE_ADX_FILTER,
        STAGE_RISK_VALIDATION, STAGE_CONFIDENCE, STAGE_IMPULSE_QUALITY
    ]
    ...
    def __init__(self, db_manager, config=None):
        ...
        if config is None:
            try:
                from forex_scanner.configdata.strategies import config_ema_double_confirmation as config
            except ImportError:
                try:
                    from configdata.strategies import config_ema_double_confirmation as config
                except ImportError:
                    config = None
        ...
        self.logger.info(f"EMADoubleRejectionHistoryManager initialized (enabled={self.enabled})")
    ...
    def _flush_batch(self) -> bool:
        ...
        try:
            ...
            insert_sql = """
                INSERT INTO ema_double_rejections (
                    ...
                    ema_fast_value, ema_slow_value, ema_trend_value, ema_fast_slow_separation_pips,
                    htf_ema_value, htf_price_position, htf_distance_pips,
                    successful_crossover_count, pending_crossover_count, last_crossover_direction, last_crossover_bars_ago,
                    adx_value, adx_trending,
                    atr_15m, atr_pips,
                    rsi_value,
                    order_type, limit_offset_pips, limit_entry_price,
            ...
            for data in self._batch_buffer:
                ...
                values = (
                    ...
                    self._safe_float(data.get('ema_fast_value')),
                    self._safe_float(data.get('ema_slow_value')),
                    self._safe_float(data.get('ema_trend_value')),
                    self._safe_float(data.get('ema_fast_slow_separation_pips')),
                    self._safe_float(data.get('htf_ema_value')),
                    data.get('htf_price_position'),
                    self._safe_float(data.get('htf_distance_pips')),
                    self._safe_int(data.get('successful_crossover_count')),
                    self._safe_int(data.get('pending_crossover_count')),
                    data.get('last_crossover_direction'),
                    self._safe_int(data.get('last_crossover_bars_ago')),
                    self._safe_float(data.get('adx_value')),
                    data.get('adx_trending'),
                    self._safe_float(data.get('atr_15m')),
                    self._safe_float(data.get('atr_pips')),
                    self._safe_float(data.get('rsi_value')),
                    data.get('order_type'),
                    self._safe_float(data.get('limit_offset_pips')),
                    self._safe_float(data.get('limit_entry_price')),
            ...
            self._batch_buffer = []
            self.logger.debug(f"Flushed {count} EMA Double rejections to database")
            return True

        except Exception as e:
            self.logger.error(f"Failed to flush EMA Double rejection batch: {e}")
            if conn:
                try:
                    conn.rollback()
                except:
        ...
        try:
            ...
            if row:
                return {
                    ...
                    'avg_adx': float(row[6]) if row[6] else 0.0,
                    'avg_crossover_count': float(row[7]) if row[7] else 0.0,
            ...
            return {
                ...
                'near_misses': 0,
                'avg_adx': 0.0,
                'avg_crossover_count': 0.0,
                'avg_rejections_per_day': 0
            }

        except Exception as e:
            self.logger.error(f"Failed to get EMA Double rejection stats: {e}")
            return {}
    ...
    def get_rejections_by_stage(self, stage: str = None, days: int = 30,
        ...
        try:
            ...
            if stage:
                cursor.execute("""
                    SELECT * FROM ema_double_rejections
            ...
            self.logger.error(f"Failed to get EMA Double rejections by stage: {e}")
            return []
        ...
        try:
            ...
            cursor.execute("""
                SELECT
                    ...
                    market_session, htf_distance_pips, adx_value,
                    successful_crossover_count, potential_risk_pips, potential_reward_pips
                FROM ema_double_rejections
            ...
            self.logger.error(f"Failed to get EMA Double near misses: {e}")
            return []
    ...
    def get_rejection_breakdown(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            result['by_stage'] = [
                {
                    ...
                    'avg_crossover_count': float(row[3]) if row[3] else 0
                }
            ...
            self.logger.error(f"Failed to get EMA Double rejection breakdown: {e}")
    ...
    def get_rejections_dataframe(self, days: int = 30, pair: str = None,
        ...
        try:
            ...
            self.logger.error(f"Failed to get EMA Double rejections dataframe: {e}")
            return pd.DataFrame()
    ...
    def cleanup_old_rejections(self, days_to_keep: int = 90) -> int:
        ...
        try:
            ...
            cursor.execute("""
                DELETE FROM ema_double_rejections
                WHERE scan_timestamp < NOW() - INTERVAL '%s days'
            """, (days_to_keep,))
            ...
            self.logger.info(f"Cleaned up {deleted} old EMA Double rejection records (keeping {days_to_keep} days)")
            return deleted

        except Exception as e:
            self.logger.error(f"Failed to cleanup old EMA Double rejections: {e}")
            if conn:
                try:
                    conn.rollback()
                except:
