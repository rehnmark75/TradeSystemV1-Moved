#!/usr/bin/env python3
"""
EURJPY Parameter Sweep v2.47.0
Current: SL=12 TP=18, max_conf=0.64, blocked=0-5,14-15,22-23
...
DAYS = 30
TIMEFRAME = '5m'
PROGRESS_FILE = '/tmp/sweep_eurjpy_progress.log'
RESULTS_FILE = '/tmp/sweep_eurjpy_results.txt'
...
def main():
    ...
    log(f"=== EURJPY Parameter Sweep ({DAYS}d, {TIMEFRAME}) — {TOTAL_TESTS} tests ===\n")
    ...
    # EURJPY 1h ATR ~15 pips. Default ref=8 is way too low.
    log("\n--- GROUP 2: ATR Reference Pips ---")
    ...
    hour_configs = [
        ('0,1,2,3,4,5,22,23,14,15', 'current'),
        ('0,1,2,3,4,5,22,23,14,15,11', 'cur+hour11'),
        ('0,1,2,3,4,5,22,23,14,15,16,17', 'cur+late NY'),
        ('0,1,2,3,4,5,22,23', 'drop overlap block'),
        ('14,15', 'overlap only'),
        ('', 'no blocks'),
    ]
