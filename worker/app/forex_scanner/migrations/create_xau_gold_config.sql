...
VALUES
    -- Identity
    ('strategy_name',          'XAU_GOLD',         'string', 'general', 'Strategy name',            1),
    ('version',                '1.0.0',            'string', 'general', 'Strategy version',         2),
    ('enabled',                'true',             'bool',   'general', 'Enable/disable strategy',  3),

    -- Timeframes
    ('htf_timeframe',          '4h',               'string', 'timeframes', 'HTF bias timeframe',   10),
    ('trigger_timeframe',      '1h',               'string', 'timeframes', 'BOS trigger timeframe',11),
    ('entry_timeframe',        '15m',              'string', 'timeframes', 'Entry timeframe',      12),

    -- Core indicators
    ('ema_fast_period',        '50',               'int',    'indicators', 'Fast EMA on HTF',      20),
    ('ema_slow_period',        '200',              'int',    'indicators', 'Slow EMA on HTF',      21),
    ('atr_period',             '14',               'int',    'indicators', 'ATR period',           22),
    ('rsi_period',             '14',               'int',    'indicators', 'RSI period',           23),
    ('adx_period',             '14',               'int',    'indicators', 'ADX period',           24),
    ('swing_lookback',         '20',               'int',    'indicators', 'Swing lookback bars',  25),

    -- Confidence
    ('min_confidence',         '0.58',             'float',  'confidence', 'Min confidence',       30),
    ('max_confidence',         '0.75',             'float',  'confidence', 'Max confidence cap (paradox)', 31),
    ('base_confidence',        '0.50',             'float',  'confidence', 'Base confidence',      32),

    -- Confluence weights (sum cap at 0.25 above base)
    ('w_htf_bias',             '0.08',             'float',  'confluence', '4H bias alignment',    40),
    ('w_bos_displacement',     '0.08',             'float',  'confluence', '1H BOS with displacement', 41),
    ('w_entry_pullback',       '0.06',             'float',  'confluence', '15m entry at untested level', 42),
    ('w_dxy_confluence',       '0.05',             'float',  'confluence', 'DXY inverse confirmation', 43),
    ('w_rsi_neutral',          '0.04',             'float',  'confluence', 'RSI 40-60 neutral band', 44),

    -- Risk: SL/TP
    ('sl_atr_multiplier',      '1.5',              'float',  'risk', 'SL as ATR multiple',         50),
    ('min_stop_loss_pips',     '25.0',             'float',  'risk', 'Floor for SL',               51),
    ('max_stop_loss_pips',     '80.0',             'float',  'risk', 'Cap for SL',                 52),
    ('rr_ratio',               '2.0',              'float',  'risk', 'Target R:R',                 53),
    ('min_rr_ratio',           '1.33',             'float',  'risk', 'Hard floor for R:R',         54),
    ('min_tp_pips',            '15.0',             'float',  'risk', 'Floor for TP',               55),
    ('fixed_sl_tp_override_enabled', 'false',      'bool',   'risk', 'Use fixed SL/TP instead of ATR', 56),
    ('fixed_stop_loss_pips',   '40.0',             'float',  'risk', 'Fixed SL fallback',          57),
    ('fixed_take_profit_pips', '80.0',             'float',  'risk', 'Fixed TP fallback',          58),
    ...
    ('session_filter_enabled', 'true',             'bool',   'session', 'Enable session filter',   70),
    ('london_start_hour',      '7',                'int',    'session', 'London session start UTC', 71),
    ('london_end_hour',        '10',               'int',    'session', 'London session end UTC',   72),
    ('ny_start_hour',          '13',               'int',    'session', 'NY session start UTC',     73),
    ('ny_end_hour',            '20',               'int',    'session', 'NY session end UTC',       74),
    ('rollover_start_hour',    '21',               'int',    'session', 'Rollover start (blocked)', 75),
    ('rollover_end_hour',      '22',               'int',    'session', 'Rollover end (blocked)',   76),
    ('asian_allowed',          'false',            'bool',   'session', 'Allow Asian session signals', 77),
    ...
    ('max_concurrent_signals', '1',                'int',    'limits', 'Max concurrent signals',  91),

    -- Filters
    ('macd_filter_enabled',    'true',             'bool',   'filters', 'MACD alignment on 1H',   100),
    ('dxy_confluence_enabled', 'true',             'bool',   'filters', 'Check DXY inverse correlation', 101),
    ('rsi_neutral_min',        '40.0',             'float',  'filters', 'RSI lower bound for neutral', 102),
    ('rsi_neutral_max',        '60.0',             'float',  'filters', 'RSI upper bound for neutral', 103),

    -- Enabled pairs (JSON array of epics)
    ('enabled_pairs',          '["CS.D.CFEGOLD.CEE.IP"]', 'json', 'pairs', 'Enabled gold epics', 110)
...
INSERT INTO xau_gold_pair_overrides (epic, pair_name, pip_size, is_enabled, is_traded, monitor_only, notes)
VALUES
    ('CS.D.CFEGOLD.CEE.IP', 'XAUUSD', 0.1, TRUE, FALSE, TRUE,
