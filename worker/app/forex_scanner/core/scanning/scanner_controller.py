...
class ScannerController:
    ...
    def _detect_raw_signals(self) -> List[Dict]:
        ...
        for epic in self.epic_list:
            try:
                signals = self._scan_single_epic(epic)
                if signals:
                    # Handle both single signal and list of signals
                    if isinstance(signals, list):
                        raw_signals.extend(signals)
                        self.logger.debug(f"✅ {len(signals)} signals detected for {epic}")
                    else:
    ...
    def _attempt_multi_timeframe_scan(self, epic: str, pair_name: str) -> Optional[Dict]:
        ...
        try:
            ...
            # Check if multi-timeframe detection method exists
            if hasattr(self.signal_detector, 'detect_signals_multi_timeframe'):
                ...
                # Fallback to all strategies if MTF not available
                signals = self.signal_detector.detect_signals_all_strategies(
                    epic, pair_name, self.spread_pips, self.default_timeframe
                )
            ...
            if signal:
                ...
                timeframes = signal.get('timeframes_analyzed', [self.default_timeframe])
    ...
    def _attempt_single_timeframe_scan(self, epic: str, pair_name: str) -> Optional[Dict]:
        """
        ✅ FIXED: Now uses detect_signals_all_strategies for comprehensive strategy coverage
        This will show logs from ALL strategies: EMA, MACD, KAMA, Zero Lag, Combined
        """
        try:
            self.logger.debug(f"🔍 Single TF scan: {epic}")
            
            # ✅ SOLUTION: Use all strategies detection instead of individual methods
            signals = self.signal_detector.detect_signals_all_strategies(
                epic, pair_name, self.spread_pips, self.default_timeframe
            )
            
            # Handle the response (could be single signal or list)
            if signals:
                # If multiple signals returned, use the highest confidence one
                if isinstance(signals, list):
                    if len(signals) == 1:
                        signal = signals[0]
                    else:
                        signal = max(signals, key=lambda s: s.get('confidence_score', 0))
                        self.logger.info(f"📊 Selected best of {len(signals)} signals for {epic}")
                else:
                    signal = signals
                ...
                signal.update({
                    ...
                    'mtf_success': False,
                    'scanning_mode': 'single_timeframe_all_strategies',
                    'timeframe': self.default_timeframe
                })
                
                return signal
            else:
                self.logger.debug(f"📊 No signals for {epic}")
                return None
    ...
    def validate_dependencies(self) -> Tuple[bool, List[str]]:
        ...
        if self.signal_detector:
            try:
                ...
                hasattr(self.signal_detector, 'detect_signals_all_strategies')
            except Exception as e:
