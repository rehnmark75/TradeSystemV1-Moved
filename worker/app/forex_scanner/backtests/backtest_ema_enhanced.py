#!/usr/bin/env python3
"""
Ema Strategy Backtest - Enhanced with Unified Framework
Run: python backtest_ema.py --epic CS.D.EURUSD.MINI.IP --days 7 --timeframe 15m
...
try:
    ...
    from core.strategies.ema_strategy import EMAStrategy, create_ema_strategy
except ImportError:
    from forex_scanner.backtests.backtest_base import BacktestBase, StandardSignal, SignalType, MarketConditions
    from forex_scanner.core.strategies.ema_strategy import EMAStrategy, create_ema_strategy
...
class EnhancedEMAStrategyBacktest(BacktestBase):
    """Enhanced Ema Strategy Backtesting with Unified Framework"""

    def __init__(self, use_optimal_parameters: bool = True):
        super().__init__('ema', use_optimal_parameters)
    ...
    def initialize_strategy(self, epic: str = None) -> EMAStrategy:
        """Initialize the ema strategy with enhanced parameters"""
        try:
            ...
            self.logger.info(f"✅ Enhanced Ema strategy initialized for {epic}")
            return strategy
    ...
    def run_strategy_backtest(self, df: pd.DataFrame, epic: str, spread_pips: float, timeframe: str) -> List[StandardSignal]:
        """
        Run enhanced ema strategy backtest on given data
        ...
        """
        try:
            self.logger.info(f"🎯 Running Enhanced Ema backtest for {epic}")
            ...
            if signal:
                ...
                standard_signal.technical_indicators.update({
                    'ema_specific_analysis': self._analyze_ema_conditions(signal, df),
                ...
                self.logger.info(f"✅ Found Enhanced Ema signal: {enhanced_signal.signal_type.value} at "
            ...
            self.logger.error(f"❌ Enhanced Ema backtest failed for {epic}: {e}")
            return []

    def _analyze_ema_conditions(self, signal: Dict, df: pd.DataFrame) -> Dict[str, Any]:
        """Analyze ema-specific market conditions"""
        try:
            if len(df) == 0:
                return {'error': 'No data available'}

            # Add ema-specific analysis here
            ...
            self.logger.error(f"Error analyzing ema conditions: {e}")
            return {'error': str(e)}
...
def main():
    """Command line interface for the enhanced ema backtest"""
    parser = argparse.ArgumentParser(description='Enhanced Ema Strategy Backtest')
    ...
    backtest = EnhancedEMAStrategyBacktest(
    ...
    try:
        ...
        if result.success:
            print(f"\n🎯 Enhanced Ema Backtest Completed Successfully!")
