...
class PreMarketService:
    ...
    async def run_premarket_scan(
        ...
        async with self.finnhub:
            ...
            # Step 8: Update ALL active scanner signals with premarket prices
            # (not just those with gaps - we want current price for all signals)
            await self._update_all_active_signals_with_premarket(quotes)
    ...
    async def _save_results(self, signals: List[PreMarketSignal]):
        ...
        # Also update stock_scanner_signals with premarket entry prices
        await self._update_scanner_signals_with_premarket(signals)

    async def _update_scanner_signals_with_premarket(self, signals: List[PreMarketSignal]):
        """
        Update active stock_scanner_signals with premarket entry prices.

        This links premarket data to existing scanner signals so the UI can display
        the updated entry price from pre-market session.
        """
        if not signals:
            return
        ...
        for signal in signals:
            try:
                # Update active signals for this ticker from today
                # that don't yet have premarket data
                update_query = """
                    UPDATE stock_scanner_signals
                    SET premarket_entry_price = $1,
                        premarket_gap_percent = $2,
                        premarket_updated_at = NOW()
                    WHERE ticker = $3
                    AND status = 'active'
                    AND DATE(signal_timestamp) = CURRENT_DATE
                """
                ...
                # Check if any rows were updated (asyncpg returns 'UPDATE N')
                if result and 'UPDATE' in result:
                    count = int(result.split()[-1])
                    if count > 0:
                        updated_count += count
                        logger.debug(
                            f"Updated {count} scanner signal(s) for {signal.symbol} "
                            f"with premarket price ${signal.quote.current_price:.2f} "
                            f"({signal.quote.gap_percent:+.1f}% gap)"
                        )
            except Exception as e:
                logger.warning(f"Failed to update scanner signal for {signal.symbol}: {e}")

        if updated_count > 0:
            logger.info(f"Updated {updated_count} scanner signals with premarket prices")

    async def _update_all_active_signals_with_premarket(self, quotes: List[PreMarketQuote]):
        """
        Update ALL active scanner signals with premarket prices.

        This ensures every active signal gets a premarket price update,
        not just those with significant gaps. This is called after the
        premarket scan to populate premarket_entry_price for all signals.
        """
        if not quotes:
            return

        # Build a lookup dict for quick access
        quote_by_ticker = {q.symbol: q for q in quotes}

        # Get all active signals from today that need premarket updates
        try:
            active_signals_query = """
                ...
                WHERE status = 'active'
                AND DATE(signal_timestamp) = CURRENT_DATE
                AND premarket_entry_price IS NULL
            """
            rows = await self.db.fetch(active_signals_query)
            tickers_needing_update = [r['ticker'] for r in rows]

            if not tickers_needing_update:
                logger.debug("No active signals need premarket price update")
                return

            logger.info(f"Found {len(tickers_needing_update)} active signals needing premarket prices")

            # For tickers we already have quotes for, update directly
            ...
            for ticker in tickers_needing_update:
                if ticker in quote_by_ticker:
                    ...
                    try:
                        ...
                        logger.warning(f"Failed to update {ticker} from cache: {e}")
                else:
                    tickers_to_fetch.append(ticker)

            if updated_from_cache > 0:
                logger.info(f"Updated {updated_from_cache} signals from cached quotes")

            # Fetch quotes for remaining tickers
            if tickers_to_fetch:
                logger.info(f"Fetching premarket quotes for {len(tickers_to_fetch)} additional tickers")
                additional_quotes = await self.finnhub.get_quotes_batch(
                    tickers_to_fetch[:50],  # Limit to avoid rate limits
                    delay_between=0.3
                )

                updated_from_fetch = 0
                for raw in additional_quotes:
                    if "error" in raw:
                        continue
                    ...
                    try:
                        update_query = """
                            ...
                            AND DATE(signal_timestamp) = CURRENT_DATE
                        """
                        await self.db.execute(update_query, current, gap_pct, ticker)
                        updated_from_fetch += 1
                    except Exception as e:
                        logger.warning(f"Failed to update {ticker}: {e}")

                if updated_from_fetch > 0:
                    logger.info(f"Updated {updated_from_fetch} signals from fetched quotes")

        except Exception as e:
            logger.error(f"Error updating active signals with premarket: {e}")
