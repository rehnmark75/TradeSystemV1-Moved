...
docker exec -it task-worker python /app/forex_scanner/monitoring/rejection_outcome_analyzer.py --days 7
    ...
    def fetch_ema_double_rejections(_self, days: int, stage_filter: str, pair_filter: str, session_filter: str) -> pd.DataFrame:
        ...
        try:
            ...
            # Table might not exist yet
            if "does not exist" in str(e):
                st.info("EMA Double Rejections table not yet created. Run the database migration first.")
            else:
                st.error(f"Error fetching EMA Double rejections: {e}")
            return pd.DataFrame()
        finally:
            conn.close()
...
docker exec -it postgres psql -U postgres -d trading -f /app/forex_scanner/migrations/create_ema_double_rejections_table.sql
        ...
        col1, col2, col3, col4 = st.columns(4)
        with col1:
            days_filter = st.selectbox("Time Period", [7, 14, 30, 60, 90], index=2, key="ema_double_rej_days")
        with col2:
        ...
        with col5:
            st.metric("Most Rejected Pair", stats.get('most_rejected_pair', 'N/A'))
        with col6:
            st.metric("Avg Crossover Count", f"{stats.get('avg_crossover_count', 0):.1f}", help="Average successful crossovers at rejection")
        ...
        with sub_tab1:
            self._render_ema_double_stage_breakdown(df, stats)

        with sub_tab2:
            self._render_ema_double_time_analysis(df)

        with sub_tab3:
            self._render_ema_double_market_context(df)

        with sub_tab4:
            self._render_ema_double_near_misses(df, days_filter)

    def _render_ema_double_stage_breakdown(self, df: pd.DataFrame, stats: dict):
    ...
    def _render_ema_double_time_analysis(self, df: pd.DataFrame):
    ...
    def _render_ema_double_market_context(self, df: pd.DataFrame):
    ...
    def _render_ema_double_near_misses(self, df: pd.DataFrame, days: int):
    ...
    def render_unfilled_orders_tab(self):
        """
        Render Unfilled Orders Analysis tab.
        Shows stop-entry orders that weren't filled and analyzes if they would have been profitable.
        """
        ...
        try:
            # Fetch summary data
            summary_query = """
            SELECT * FROM v_unfilled_order_summary
            """
            ...
            WHERE symbol NOT LIKE '%CEEM%'  -- Exclude CEEM due to different pricing
            ...
            """
            epic_df = pd.read_sql_query(epic_query, conn)

        except Exception as e:
            st.error(f"Error fetching unfilled order data: {e}")
        ...
        with sub_tab1:
            ...
            for idx, row in detail_df.iterrows():
                ...
                icon = "✅" if quality == 'GOOD_SIGNAL' else "❌" if quality == 'BAD_SIGNAL' else "⏳"
                with st.expander(f"{icon} {row['symbol']} {row['direction']} @ {row['order_time']}"):
                    ...
                    if quality == 'GOOD_SIGNAL':
                        ...
                        st.warning("⏳ INCONCLUSIVE: Neither TP nor SL hit in 24h. More data needed.")
                ...
                for _, row in epic_df.iterrows():
                    ...
                    with st.expander(f"📊 {epic} ({total} unfilled)"):
                        ...
                        if avg_gap and avg_gap > 5:
                            issues.append(f"⚠️ High avg gap to entry: {avg_gap} pips")
                            recommendations.append("Consider reducing stop-entry offset or extending expiry")

                        if bad > good and (good + bad) > 0:
                            issues.append(f"⚠️ More bad signals ({bad}) than good ({good})")
