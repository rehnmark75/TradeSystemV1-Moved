...
export async function GET(request: Request, { params }: { params: { epic: string } }) {
  try {
    ...
    console.error("Failed to load FA_OR_ATR_TRAIL pair override", error);
    return NextResponse.json({ error: "Failed to load FA_OR_ATR_TRAIL pair override" }, { status: 500 });
  }
}
...
export async function PUT(request: Request, { params }: { params: { epic: string } }) {
  ...
  try {
    ...
    const currentResult = await client.query(
      `
        SELECT * FROM fa_or_atr_trail_pair_overrides
    ...
    const result = await client.query(
      `
        UPDATE fa_or_atr_trail_pair_overrides
    ...
    await client.query("ROLLBACK");
    console.error("Failed to update FA_OR_ATR_TRAIL pair override", error);
    return NextResponse.json({ error: "Failed to update FA_OR_ATR_TRAIL pair override" }, { status: 500 });
  } finally {
    client.release();
  }
}
...
export async function DELETE(request: Request, { params }: { params: { epic: string } }) {
  ...
  try {
    await strategyConfigPool.query(
      `DELETE FROM fa_or_atr_trail_pair_overrides WHERE config_set = $1 AND epic = $2`,
      [configSet, params.epic],
    );
    return NextResponse.json({ success: true });
  } catch (error) {
    console.error("Failed to delete FA_OR_ATR_TRAIL pair override", error);
    return NextResponse.json({ error: "Failed to delete FA_OR_ATR_TRAIL pair override" }, { status: 500 });
  }
}
