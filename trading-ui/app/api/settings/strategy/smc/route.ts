...
import { validateSmcUpdates } from "../../../../../lib/validation/settingsSchemas";
...
export async function GET() {
  try {
    ...
    console.error("Failed to load SMC config", error);
    return NextResponse.json(
      { error: "Failed to load SMC config" },
      { status: 500 }
    );
  }
}
...
export async function PATCH(request: Request) {
  ...
  const validation = validateSmcUpdates(updates);
  ...
  try {
    ...
    await client.query("ROLLBACK");
    console.error("Failed to update SMC config", error);
    return NextResponse.json(
      { error: "Failed to update SMC config" },
      { status: 500 }
    );
  } finally {
    client.release();
