...
export async function GET(request: Request, { params }: { params: { epic: string } }) {
  try {
    ...
    const globalConfig = { ...KAMA2_GLOBAL_DEFAULTS };
    ...
    console.error("Failed to load effective KAMA_V2 config", error);
    return NextResponse.json({ error: "Failed to load effective KAMA_V2 config" }, { status: 500 });
  }
}
