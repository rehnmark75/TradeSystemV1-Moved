...
export async function GET(request: Request, { params }: { params: { epic: string } }) {
  try {
    ...
    console.error("Failed to load KAMA_V2 pair override", error);
    return NextResponse.json({ error: "Failed to load KAMA_V2 pair override" }, { status: 500 });
  }
}
...
export async function PUT(request: Request, { params }: { params: { epic: string } }) {
  ...
  try {
    ...
    await client.query("ROLLBACK");
    console.error("Failed to update KAMA_V2 pair override", error);
    return NextResponse.json({ error: "Failed to update KAMA_V2 pair override" }, { status: 500 });
  } finally {
    client.release();
  }
}
...
export async function DELETE(request: Request, { params }: { params: { epic: string } }) {
  ...
  try {
    ...
    console.error("Failed to delete KAMA_V2 pair override", error);
    return NextResponse.json({ error: "Failed to delete KAMA_V2 pair override" }, { status: 500 });
  }
}
