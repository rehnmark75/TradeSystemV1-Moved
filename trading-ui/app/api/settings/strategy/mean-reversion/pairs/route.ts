...
export async function GET(request: Request) {
  try {
    ...
    console.error("Failed to load MEAN_REVERSION pair overrides", error);
    return NextResponse.json(
      { error: "Failed to load MEAN_REVERSION pair overrides" },
      { status: 500 },
    );
  }
}
...
export async function POST(request: Request) {
  ...
  const { epic, updates, overrides, updated_by, change_reason, config_set } = body as {
    epic?: string;
  ...
  if (!epic || !updated_by || !change_reason) {
    return NextResponse.json(
      { error: "epic, updated_by, and change_reason are required" },
      { status: 400 },
    );
  }
  ...
  try {
    const result = await strategyConfigPool.query(
      `
        INSERT INTO mean_reversion_pair_overrides (${columns.join(", ")})
    ...
    console.error("Failed to upsert MEAN_REVERSION pair override", error);
    return NextResponse.json(
      { error: "Failed to upsert MEAN_REVERSION pair override" },
      { status: 500 },
    );
  }
}
