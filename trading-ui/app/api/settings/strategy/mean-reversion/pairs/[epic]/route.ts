...
export async function GET(request: Request, { params }: { params: { epic: string } }) {
  try {
    ...
    console.error("Failed to load MEAN_REVERSION pair override", error);
    return NextResponse.json({ error: "Failed to load MEAN_REVERSION pair override" }, { status: 500 });
  }
}
...
export async function PUT(request: Request, { params }: { params: { epic: string } }) {
  ...
  try {
    ...
    await client.query("ROLLBACK");
    console.error("Failed to update MEAN_REVERSION pair override", error);
    return NextResponse.json({ error: "Failed to update MEAN_REVERSION pair override" }, { status: 500 });
  } finally {
    client.release();
  }
}
...
export async function DELETE(request: Request, { params }: { params: { epic: string } }) {
  ...
  try {
    await strategyConfigPool.query(
      `
        DELETE FROM mean_reversion_pair_overrides
        WHERE config_set = $1 AND epic = $2
      `,
      [configSet, params.epic],
    );
    ...
    console.error("Failed to delete MEAN_REVERSION pair override", error);
    return NextResponse.json({ error: "Failed to delete MEAN_REVERSION pair override" }, { status: 500 });
  }
}
