  ...
  try {
    ...
    const monitorOnly =
      override && override.monitor_only !== null && override.monitor_only !== undefined
        ...
        : Boolean(globalConfig.monitor_only ?? true);
    ...
    if (override) {
      ...
      Object.entries(override).forEach(([key, value]) => {
        if (
          ["id", "config_set", "epic", "pair_name", "created_at", "updated_at", "parameter_overrides", "notes"].includes(
            key,
          )
        ) {
          return;
    ...
    console.error("Failed to load effective SMC_MOMENTUM config", error);
    return NextResponse.json({ error: "Failed to load effective SMC_MOMENTUM config" }, { status: 500 });
  }
}
