...
async function loadRows(configSet: string, client?: { query: Function }): Promise<KVRow[]> {
  ...
  const result = await executor.query(
    `SELECT * FROM smc_momentum_global_config WHERE config_set = $1 AND is_active = TRUE ORDER BY id`,
    [configSet],
  );
  return result.rows;
}
  ...
  try {
    ...
    if (!rows.length) {
      return NextResponse.json(
        { error: `No active SMC_MOMENTUM config found for config_set='${configSet}'` },
        { status: 404 },
      );
    }
    return NextResponse.json({ ...pivotRows(rows), strategy_name: "SMC_MOMENTUM" });
  } catch (error) {
    console.error("Failed to load SMC_MOMENTUM config", error);
    return NextResponse.json({ error: "Failed to load SMC_MOMENTUM config" }, { status: 500 });
  }
}
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    const maxTs = rows.reduce(
      (max, r) => {
        const ts = r.updated_at instanceof Date ? r.updated_at : new Date(String(r.updated_at));
    ...
    if (normalizeTimestamp(maxTs) !== updated_at) {
      await client.query("ROLLBACK");
      return NextResponse.json(
        { error: "conflict", message: "Settings were updated by another user", current_config: pivotRows(rows) },
        { status: 409 },
      );
    }

    const existingParams = new Set(rows.map((r) => r.parameter_name));
    const keys = Object.keys(updates).filter((k) => existingParams.has(k));
    if (!keys.length) {
      await client.query("ROLLBACK");
    ...
    return NextResponse.json({ ...pivotRows(updatedRows), strategy_name: "SMC_MOMENTUM" });
  } catch (error) {
    await client.query("ROLLBACK");
    console.error("Failed to update SMC_MOMENTUM config", error);
    return NextResponse.json({ error: "Failed to update SMC_MOMENTUM config" }, { status: 500 });
  } finally {
    client.release();
  }
}
