...
export async function GET(request: Request, { params }: { params: { epic: string } }) {
  try {
    ...
    const result = await strategyConfigPool.query(
      `SELECT * FROM smc_momentum_pair_overrides WHERE config_set = $1 AND epic = $2 LIMIT 1`,
    ...
    console.error("Failed to load SMC_MOMENTUM pair override", error);
    return NextResponse.json({ error: "Failed to load SMC_MOMENTUM pair override" }, { status: 500 });
  }
}
...
export async function PUT(request: Request, { params }: { params: { epic: string } }) {
  ...
  try {
    ...
    const currentResult = await client.query(
      `
        SELECT * FROM smc_momentum_pair_overrides
    ...
    await client.query("ROLLBACK");
    console.error("Failed to update SMC_MOMENTUM pair override", error);
    return NextResponse.json({ error: "Failed to update SMC_MOMENTUM pair override" }, { status: 500 });
  } finally {
    client.release();
  }
}
...
export async function DELETE(request: Request, { params }: { params: { epic: string } }) {
  ...
  try {
    await strategyConfigPool.query(
      `DELETE FROM smc_momentum_pair_overrides WHERE config_set = $1 AND epic = $2`,
      [configSet, params.epic],
    );
    return NextResponse.json({ success: true });
  } catch (error) {
    console.error("Failed to delete SMC_MOMENTUM pair override", error);
    return NextResponse.json({ error: "Failed to delete SMC_MOMENTUM pair override" }, { status: 500 });
  }
}
