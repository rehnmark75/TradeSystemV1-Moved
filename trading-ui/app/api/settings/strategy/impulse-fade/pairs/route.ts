...
export async function GET() {
  try {
    const result = await strategyConfigPool.query(
      `SELECT * FROM impulse_fade_pair_overrides ORDER BY epic ASC`,
    );
    return NextResponse.json({ overrides: result.rows ?? [] });
  } catch (error) {
    console.error("Failed to load IMPULSE_FADE pair overrides", error);
    return NextResponse.json({ error: "Failed to load IMPULSE_FADE pair overrides" }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    const result = await strategyConfigPool.query(
      `
        INSERT INTO impulse_fade_pair_overrides (${columns.join(", ")})
    ...
    console.error("Failed to upsert IMPULSE_FADE pair override", error);
    return NextResponse.json({ error: "Failed to upsert IMPULSE_FADE pair override" }, { status: 500 });
  }
}
