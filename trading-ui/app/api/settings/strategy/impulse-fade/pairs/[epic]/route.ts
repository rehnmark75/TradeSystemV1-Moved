...
async function getOverrideColumns(client?: { query: Function }): Promise<string[]> {
  ...
  const result = await executor.query(
    `SELECT column_name FROM information_schema.columns WHERE table_name = 'impulse_fade_pair_overrides'`,
  );
...
export async function GET(_request: Request, { params }: { params: { epic: string } }) {
  try {
    const result = await strategyConfigPool.query(
      `SELECT * FROM impulse_fade_pair_overrides WHERE epic = $1 LIMIT 1`,
      [params.epic],
    );
    if (!result.rows[0]) {
    ...
    console.error("Failed to load IMPULSE_FADE pair override", error);
    return NextResponse.json({ error: "Failed to load IMPULSE_FADE pair override" }, { status: 500 });
  }
}
...
export async function PUT(request: Request, { params }: { params: { epic: string } }) {
  ...
  try {
    ...
    const result = await client.query(
      `
        UPDATE impulse_fade_pair_overrides
    ...
    await client.query("ROLLBACK");
    console.error("Failed to update IMPULSE_FADE pair override", error);
    return NextResponse.json({ error: "Failed to update IMPULSE_FADE pair override" }, { status: 500 });
  } finally {
    client.release();
  }
}
...
export async function DELETE(_request: Request, { params }: { params: { epic: string } }) {
  try {
    await strategyConfigPool.query(
      `DELETE FROM impulse_fade_pair_overrides WHERE epic = $1`,
      [params.epic],
    );
    return NextResponse.json({ success: true });
  } catch (error) {
    console.error("Failed to delete IMPULSE_FADE pair override", error);
    return NextResponse.json({ error: "Failed to delete IMPULSE_FADE pair override" }, { status: 500 });
  }
}
