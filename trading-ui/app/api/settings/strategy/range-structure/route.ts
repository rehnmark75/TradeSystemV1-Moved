...
async function loadActiveConfig(client?: any) {
...
async function loadActiveConfigForUpdate(client: any) {
...
async function getGlobalColumns(client?: any): Promise<Set<string>> {
  ...
  return new Set(
    result.rows
      ...
      .filter(
        (col: string) =>
          !["id", "created_at", "updated_at", "is_active", "notes"].includes(col),
      ),
  );
}
...
export async function GET() {
  try {
    ...
    return NextResponse.json({ ...config, strategy_name: "RANGE_STRUCTURE" });
  } catch (error) {
    console.error("Failed to load RANGE_STRUCTURE config", error);
    return NextResponse.json({ error: "Failed to load RANGE_STRUCTURE config" }, { status: 500 });
  }
}
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    if (!current) {
      ...
      return NextResponse.json({ error: "No active RANGE_STRUCTURE config found" }, { status: 404 });
    }
    ...
    const result = await client.query(
      `
        UPDATE range_structure_global_config
        SET ${setClauses.join(", ")},
            updated_at = NOW()
    ...
    return NextResponse.json({ ...result.rows[0], strategy_name: "RANGE_STRUCTURE" });
  } catch (error) {
    await client.query("ROLLBACK");
    console.error("Failed to update RANGE_STRUCTURE config", error);
    return NextResponse.json({ error: "Failed to update RANGE_STRUCTURE config" }, { status: 500 });
  } finally {
    client.release();
  }
}
