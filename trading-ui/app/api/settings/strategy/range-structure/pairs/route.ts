...
export async function GET() {
  try {
    ...
    console.error("Failed to load RANGE_STRUCTURE pair overrides", error);
    return NextResponse.json(
      { error: "Failed to load RANGE_STRUCTURE pair overrides" },
      { status: 500 },
    );
  }
}
...
export async function POST(request: Request) {
  ...
  const allowed = new Set(
    (await getOverrideColumns()).filter(
      (column: string) =>
  ...
  try {
    const result = await strategyConfigPool.query(
      `
        INSERT INTO range_structure_pair_overrides (${columns.join(", ")})
    ...
    console.error("Failed to upsert RANGE_STRUCTURE pair override", error);
    return NextResponse.json(
      { error: "Failed to upsert RANGE_STRUCTURE pair override" },
      { status: 500 },
    );
  }
}
