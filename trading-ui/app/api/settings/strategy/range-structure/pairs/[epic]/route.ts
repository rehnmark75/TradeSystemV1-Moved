...
export async function GET(_request: Request, { params }: { params: { epic: string } }) {
  try {
    ...
    console.error("Failed to load RANGE_STRUCTURE pair override", error);
    return NextResponse.json({ error: "Failed to load RANGE_STRUCTURE pair override" }, { status: 500 });
  }
}
...
export async function PUT(request: Request, { params }: { params: { epic: string } }) {
  ...
  try {
    ...
    await client.query("ROLLBACK");
    console.error("Failed to update RANGE_STRUCTURE pair override", error);
    return NextResponse.json({ error: "Failed to update RANGE_STRUCTURE pair override" }, { status: 500 });
  } finally {
    client.release();
  }
}
...
export async function DELETE(_request: Request, { params }: { params: { epic: string } }) {
  try {
    await strategyConfigPool.query(
      `
        DELETE FROM range_structure_pair_overrides
        WHERE epic = $1
      `,
      [params.epic],
    );
    ...
    console.error("Failed to delete RANGE_STRUCTURE pair override", error);
    return NextResponse.json({ error: "Failed to delete RANGE_STRUCTURE pair override" }, { status: 500 });
  }
}
