...
const CATEGORY_MAP: Record<string, string> = {
  entry_bars: "Channel Parameters",
  exit_bars: "Channel Parameters",
  atr_period: "Channel Parameters",
  atr_stop_multiplier: "Channel Parameters",
  long_only: "Direction",
...
const HIDDEN = new Set(["id", "strategy_name", "version", "is_active", "created_at", "updated_at"]);
...
export async function GET() {
  try {
    const result = await strategyConfigPool.query(
      `SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'donchian_turtle_global_config' ORDER BY ordinal_position`,
    );
    ...
    console.error("Failed to load DONCHIAN_TURTLE metadata", error);
    return NextResponse.json({ error: "Failed to load DONCHIAN_TURTLE metadata" }, { status: 500 });
  }
}
