...
async function loadActiveConfigForUpdate(client: { query: Function }) {
  const result = await client.query(
    `SELECT * FROM donchian_turtle_global_config WHERE is_active = TRUE ORDER BY updated_at DESC LIMIT 1 FOR UPDATE`,
  );
  return result.rows[0] ?? null;
}
...
async function getGlobalColumns(client?: { query: Function }): Promise<Set<string>> {
  const executor = client ?? strategyConfigPool;
  const result = await executor.query(
    `SELECT column_name FROM information_schema.columns WHERE table_name = 'donchian_turtle_global_config'`,
  );
  return new Set(
    result.rows
      .map((row: { column_name: string }) => row.column_name)
      .filter((col: string) => !["id", "strategy_name", "version", "created_at", "updated_at", "is_active"].includes(col)),
  );
}
...
export async function GET() {
  try {
    ...
    return NextResponse.json({ ...config, strategy_name: "DONCHIAN_TURTLE" });
  } catch (error) {
    console.error("Failed to load DONCHIAN_TURTLE config", error);
    return NextResponse.json({ error: "Failed to load DONCHIAN_TURTLE config" }, { status: 500 });
  }
}
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    const result = await client.query(
      `UPDATE donchian_turtle_global_config SET ${setClauses.join(", ")}, updated_at = NOW() WHERE id = $${keys.length + 1} RETURNING *`,
      values,
    );

    await client.query("COMMIT");
    return NextResponse.json({ ...result.rows[0], strategy_name: "DONCHIAN_TURTLE" });
  } catch (error) {
    await client.query("ROLLBACK");
    console.error("Failed to update DONCHIAN_TURTLE config", error);
    return NextResponse.json({ error: "Failed to update DONCHIAN_TURTLE config" }, { status: 500 });
  } finally {
    client.release();
  }
}
