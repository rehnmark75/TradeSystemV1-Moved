...
export async function GET(request: Request) {
  try {
    ...
    console.error("Failed to load INSIDE_DAY pair overrides", error);
    return NextResponse.json({ error: "Failed to load INSIDE_DAY pair overrides" }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.error("Failed to upsert INSIDE_DAY pair override", error);
    return NextResponse.json({ error: "Failed to upsert INSIDE_DAY pair override" }, { status: 500 });
  }
}
